<template>
    <div>
      ...
      <select v-model="pickupCity" id="pickupCity">
        <option v-for="city in cities" :key="city">{{ city }}</option>
      </select>
  
      <label for="deliveryCity">Delivery City:</label>
      <select v-model="deliveryCity" id="deliveryCity">
        <option v-for="city in cities" :key="city">{{ city }}</option>
      </select>
  
      <button @click="calculateDeliveryInfo">Calculate Delivery</button>
  
      <div v-if="deliveryInfo">
        <p>Estimated Distance: {{ deliveryInfo.distance.toFixed(2) }} km</p>
        <p>Estimated Delivery Time: {{ deliveryInfo.deliveryTime.toFixed(2) }} hours</p>
      </div>
    </div>
  </template>
    ...
    data() {
      return {
        ...
        cityCoordinates: {
          ...
          Jelgava: [56.6500, 23.7128],
          Jūrmala: [56.9475, 23.6160],
